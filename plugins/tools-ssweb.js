...
var handler = async (m, { conn, command, args }) => {
  ...
  try {
    var img = await fetch(`https://xzn.wtf/api/ssweb?type=dekstop&url=${url}&apikey=Akiraabotz`);
    if (!img) {
      await m.reply('Gagal saat percobaan pertama. Memulai percobaan kedua...');
      img = await fetch(`https://xzn.wtf/api/ssweb?type=dekstop&url=${url}&apikey=Akiraabotz`);
